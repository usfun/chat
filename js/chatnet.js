    ...
    if(message_data != ""){
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $("[data-random-id='" + data.random_id + "']").find('.message-time').html(moment(data.time+system_timezone_offset).tz(user_timezone).format('A hh:mm'));
    ...
    $.ajax({
        ...
        success: function(data) {
            if (Object.keys(data.stickers).length > 0) {
                ...
                $('.sticker-tab-content').append('<p class="text-center">{{_("未知的贴纸")}}</p>');
            }
        },complete: function(){
        ...
        success: function(data) {
            ...
            if (data.info_type == "user") {
                ...
                if (display_name_format == 'username') {
                    ...
                    var display_name = data.info.first_name;
                }
                ...
                if(data.info.about){
                    ...
                    $('.active-user-about').html('主人很懒，没有填写个人说明！');
                }
                ...
                if(user_show==false){
                    if(data.info.blocked_by_you){
                        restrictTypingArea(data.info.blocked_by_you, "您已屏蔽此对话");
                    }else if (data.info.blocked_by_him) {
                        restrictTypingArea(data.info.blocked_by_him, "此对话已被对方屏蔽");
                    }else{
                ...
                if(data.info.room_data.description){
                    $('.active-group-description').html(data.info.room_data.description);
                }else{
                    $('.active-group-description').html('群主太懒了，这就是群说明！');
                }
                $('.active-group-notice').html(data.info.room_data.notice);
                ...
                $.each(data.group_users, function( index, obj ) {
                    ...
                    if (obj.user_type != "") {
                        if (obj.user_type == 1) {
                            user_type = "<span class='user-type-badge admin'>柠萌</span>";
                        }else if(obj.user_type == 4){
                            user_type = "<span title='{{_('Global Moderator')}}' class='user-type-badge mod'>房管</span>";
                        }else if(obj.user_type == 2){
                            var active_room_created_by = $('#active_room_created_by').val();
                            if(active_room_created_by == obj.id){
                                user_type = "<span title='{{_('Room Creator')}}' class='user-type-badge creator'>群主</span>";
                            }else if(obj.is_mod == 1){
                                user_type = "<span title='{{_('Room Moderator')}}' class='user-type-badge room-mod'>管理员</span>";
                            }
                        }
                    }
                    {% endif %}
                    ...
                    var each_user = '<div class="row group-user" data-user-id="'+obj.id+'"><div class="col-12"><img class="img-profile mr-2" src='+img_src+'>' + user_type +' <div class="grp-user-name">' + display_name + `</div></span> ` + sex + ` ` + country + ` `+ '</div></div>';
                                    ...
                                    <div class="file-description">
                                        ...
                                        <div class="file-meta">
                                            <div class="file-meta-entry">
                                                <div class="file-meta-swap">`+file_obj.size+` `+file_obj.extenstion+` 文件</div>
                                            </div>
                                        </div>
                                    </div>
            ...
            $.each(data.shared_links, function(all_links_idx, link_obj) {
                ...
                if (!link_obj.image) {
                    var img_link = '{{STATIC_URL}}/img/default-image-link.png';
                }else{
                    ...
                    <div class="file-section">
                        <a href="`+link_obj.url+`" target="_blank" class="file-header">
                            ...
                            <div class="file-description">
                                ...
                                <div class="file-meta">
                                    <div class="file-meta-entry">
                                        <div class="file-meta-swap">`+link_obj.message+` 文件</div>
                                    </div>
                                </div>
                            </div>
...
function forward_list_create(forward_msg_id){
    ...
    $('.forward-selection-name').html('选择 ' + forward_msg_list.length + ' 条消息');
...
function getLinkPreview(json_msg){
    if (json_msg.image) {
        ...
        var preview_image = `{{STATIC_URL}}/img/default-image-link.png`;
    }
    if (json_msg.title) {
    ...
    var vurl = json_msg.url;
    var hostname = a.hostname;
    
    if(vurl.indexOf(".m3u8") > -1 || vurl.indexOf(".mp4") > -1 || vurl.indexOf("bilibili.com") > -1 || vurl.indexOf("qq.com") > -1 || vurl.indexOf("iqiyi.com") > -1 || vurl.indexOf("youku.com") > -1 || vurl.indexOf("mgtv.com") > -1 || vurl.indexOf("sohu.com") > -1 || vurl.indexOf("cctv.com") > -1 || vurl.indexOf("pptv.com") > -1 || vurl.indexOf("le.com") > -1 ){
        link_class = 'video-link';
        
    }else if (hostname == 'www.youtube.com' || hostname == 'youtube.com' || hostname == 'api.leeleo.cn') {
        link_class = 'video-link';
    }
        ...
        var link_preview = `<div class="chat-link-block">` +
            `<a target="_blank" href="`+json_msg.url+`">` +
                `<div class="link-preview `+link_class+`">` +
                        ...
                        `<img class="lazy" src="{{STATIC_URL}}/img/default-video-link.png" data-src="`+preview_image+`" />` +
                        `<div class="link-meta">` +
                            preview_title +
                            preview_description +
                        `</div>` +
...
function createMessage(obj, direction="down", save_message=false){
    ...
    if(obj.status != 3){
        msg = messageHtml(obj);
        msg_forward_btn = `<i class="fa fa-share message-forward" title="{{_('转发')}}"></i>`;
        ...
        msg_reply_btn = `<i class="fa fa-reply message-reply" data-chat-type="`+obj.chat_type+`" title="{{_('回复')}}"></i>`;

    }else{
        deleted = "deleted";
        msg_status = "";
        msg = `<div class="chat-txt deleted"><i class="fa fa-ban"></i> {{_('消息已撤回')}}</div>`;
        msg_forward_btn = ``;
        msg_reply_btn = ``;
    }

    var new_chat_date = moment(obj.time+system_timezone_offset).tz(user_timezone).format('YYYY-M-D dddd');
    var msg_content = `<li class="`+message_class_name+`" id="`+obj.id+`" data-random-id="`+obj.random_id+`" data-msg-type="`+obj.type+`" data-date="`+new_chat_date+`">
                            ...
                            <span class="message-meta">
                                ...
                                <span class="message-time">`+ moment(obj.time+system_timezone_offset).tz(user_timezone).format('A hh:mm') +`</span>
                            </span>
                        </li>`;
    ...
    $(".messages  ul").find(`[data-date='`+new_chat_date+`']:first`).before(`<li class="new-date" style="margin-bottom:20px;" data-printed-date="`+new_chat_date+`"><p>`+new_chat_date+`</p></li>`);
    ...
    $(".message-data,.avatar").on('click', function() {
        $(".chat-actions").removeClass("chat-actions-show");
        $(this).parent("li").children(".chat-actions").addClass("chat-actions-show");
        setTimeout(function(){$(this).parent("li").children(".chat-actions").removeClass("chat-actions-show");}.bind(this),2500);
    ...
    if(obj.type == 1){
        ...
        if (images.length > 2) {
            ...
            $.each(JSON.parse(obj.message), function(img_idx, img_obj) {
                ...
                each_img = `<figure>
                                <a href="{{MEDIA_URL}}/chats/images/large/`+img_obj+`" data-size="`+image_size+`">
                                    <img width="`+thumb_width+`" height="`+thumb_height+`" class="lazy" data-src="{{MEDIA_URL}}/chats/images/thumb/`+img_obj+`" src="{{STATIC_URL}}/img/loading.gif" />
                                </a>
                            </figure>`;
                msg = msg + each_img;
            });
        ...
        msg = `<div class="chat-gif"> <img class="lazy" data-src="`+obj.message+`" src="{{STATIC_URL}}/img/loading.gif"/> </div>`;
    }else if (obj.type == 4){
        msg = `<div class="chat-sticker"> <img  class="lazy" data-src="{{MEDIA_URL}}/stickers/`+encodeURI(obj.message)+`"  src="{{STATIC_URL}}/img/loading.gif"/> </div>`;
    }else if(obj.type == 5){
                ...
                <div class="file-section">
                    <a href="#" class="file-header">
                        ...
                        <div class="file-description">
                            ...
                            <div class="file-meta">
                                <div class="file-meta-entry">
                                    <div class="file-meta-swap">`+file_obj.size+` `+file_obj.extenstion+` {{_('文件')}}</div>
                                </div>
                            </div>
                        </div>
        ...
        var current_message = (emojione.shortnameToImage(linkParse(htmlEncode(replied_data['current_message']))));
        var current_preview = replied_data['current_preview'];

        if(json_msg.reply_message.reply_from_id == {{ USER.id }}){
            var replied_to = "{{_('你的消息')}}";
        }else{
            var replied_to = json_msg.reply_message.reply_from + " {{_('的消息')}}";
        }
        ...
        msg_obj['id'] = "";
        var new_msg = messageHtml(msg_obj);
        msg = `<div class="chat-fwd"><span class="fwd-label"><i class="fa fa-share"></i> 转发</span>` + new_msg + `</div>`;
    }
...
function repliedMessage(current_message, current_message_type){
    ...
    if(current_message_type == 1){
        ...
        if (images.length > 1) {
            replied_html = "<i class='fa fa-image'></i> "+images.length+" {{_('多个图片')}}";
        }else{
            replied_html = "<i class='fa fa-image'></i> {{_('图片')}}";
        }
        ...
        replied_html = "<i class='fa fa-smile'></i> {{_('贴纸')}}";
        ...
        if (files.length > 1) {
            replied_html = "<i class='fa fa-file-alt'></i> "+files.length+" {{_('多个文件')}}";
        }else{
            replied_html = "<i class='fa fa-file-alt'></i> {{_('文件')}}";
        }
        ...
        replied_html = "<i class='fa fa-microphone-alt'></i> "+duration+" {{_('语音消息')}}";
        ...
        success: function(data) {
            if(data.success){
                ...
                if(data.type == "is_blocked"){
                    restrictTypingArea(data.status, "此对话已被对方屏蔽");
                }
            }
        }
    });
        ...
        $.ajax({
            ...
            success: function(data) {
                $.each(data.chats, function( index, obj ) {
                    ...
                    res_chat_time = res_chat_time + " - " + moment(obj.time+system_timezone_offset).tz(user_timezone).format('YYYY-M-D dddd');
                    if (obj.sender_id == {{USER.id}}) {
                        var res_name = '你';
                    }else{
                        if (display_name_format == 'username') {
                    ...
                    if (obj.type==1) {
                        ...
                        res_msg = "<i class='fa fa-image'></i> {{_('分享图片')}}";
                    }else if(obj.type == 3){
                        res_msg = "<i class='fa fa-image'></i> {{_('分享GIF')}}";
                    }else if(obj.type == 4){
                        res_msg = "<i class='fa fa-smile'></i> {{_('分享贴图')}}";
                    }else if(obj.type == 5){
                        res_msg = "<i class='fa fa-link'></i> {{_('分享链接')}}";
                    }else if(obj.type == 6){
                        res_msg = "<i class='fa fa-file-alt'></i> {{_('发送文件')}}";
                    }else if(obj.type == 7){
                        res_msg = "<i class='fa microphone-alt'></i> {{_('发送语音消息')}}";
                    }else if(obj.type == 8){
                        if(JSON.parse(obj.message)['new_message']['new_type'] == 1){
                            res_msg = "回复 : "+JSON.parse(obj.message)['new_message']['new_content'];
                        }else if(JSON.parse(obj.message)['new_message']['new_type'] == 2){
                            res_msg = "回复 : <i class='fa fa-image'></i> {{_('分享图片')}}";
                        }else if(JSON.parse(obj.message)['new_message']['new_type'] == 3){
                            res_msg = "回复 : <i class='fa fa-image'></i> {{_('分享GIF')}}";
                        }else if(JSON.parse(obj.message)['new_message']['new_type'] == 4){
                            res_msg = "回复 : <i class='fa fa-smile'></i> {{_('分享贴图')}}";
                        }else if(JSON.parse(obj.message)['new_message']['new_type'] == 5){
                            res_msg = "回复 : <i class='fa fa-link'></i> {{_('分享链接')}}";
                        }else if(JSON.parse(obj.message)['new_message']['new_type'] == 6){
                            res_msg = "回复 : <i class='fa fa-file-alt'></i> {{_('发送文件')}}";
                        }else if(JSON.parse(obj.message)['new_message']['new_type'] == 7){
                            res_msg = "回复 : <i class='fa microphone-alt'></i> {{_('发送语音消息')}}";
                        }
                    }
                    var result =
...
function createOnlineUser(obj){
    ...
    if (obj.user_type != "") {
        if (obj.user_type == 1) {
            ...
            user_type = "<span class='user-type-badge mod'>房管</span>";
        }else if(obj.user_type == 2){
            var active_room_created_by = $('#active_room_created_by').val();
            if(active_room_created_by == obj.user_id){
                user_type = "<span class='user-type-badge creator'>群主</span>";
            }else if(obj.is_mod == 1){
                user_type = "<span class='user-type-badge room-mod'>管理员</span>";
            }
        }
    }
    {% endif %}
    ...
    if(!obj.blocked_by_him && !obj.blocked_by_you){
        if(obj.last_message_status == 3){
            last_msg = "<i class='fa fa-ban'></i> {{_('')}}消息被撤回";
        }else if(obj.last_message_type == 1){
            last_msg = (emojione.shortnameToImage(linkParse(htmlEncode(obj.last_message))));
        }else if(obj.last_message_type == 2){
            last_msg = "<i class='fa fa-image'></i> {{_('图片')}}";
        }else if(obj.last_message_type == 3){
            last_msg = "<i class='fa fa-image'></i> {{_('GIF')}} ";
        }else if(obj.last_message_type == 4){
            last_msg = "<i class='fa fa-smile'></i> {{_('贴纸')}}";
        }else if(obj.last_message_type == 5){
            last_msg = "<i class='fa fa-link'></i> {{_('链接')}}";
        }else if(obj.last_message_type == 6){
            last_msg = "<i class='fa fa-file-alt'></i> {{_('文件')}}";
        }else if(obj.last_message_type == 7){
            last_msg = "<i class='fa microphone-alt'></i> {{_('语音')}} ";
        }else if(obj.last_message_type == 8){
            last_msg = "<i class='fa fa-reply'></i> {{_('回复消息')}} ";
        }else if(obj.last_message_type == 9){
            last_msg = "<i class='fa fa-share'></i> {{_('转发消息')}} ";
        }else{
            if (display_name_format == 'username') {
            	var display_name = obj.user_name;
            }else{
                ...
                <div class="user-options">
                    <div class="dropdown">
                      ...
                      <div class="dropdown-menu" >
                        <a class="dropdown-item view-profile" data-user-id="` + obj.user_id + `" href="#">{{_('查看信息')}}</a>
                      </div>
                    </div>
                </div>
            </div>
...
function createForwardUser(obj){
    ...
    var list_user_timezone = obj.timzone;
    var user_status_class = "offline";
    var user_status_class_name = "离线";

    if(obj.online_status>0){
        if(obj.user_status == 1){
            user_status_class = "online";
            user_status_class_name = "在线";
        }else if(obj.user_status == 2){
            user_status_class = "offline";
            user_status_class_name = "离线";
        }else if(obj.user_status == 3){
            user_status_class = "busy";
            user_status_class_name = "忙碌";
        }else if(obj.user_status == 4){
            user_status_class = "away";
            user_status_class_name = "离开";
        }
    }else{
        user_status_class = "offline";
    }
    ...
    `<label id="" class="recent-chat " for="` + obj.user_id + `_0_chat" data-id="` + obj.user_id + `" data-name="` + display_name + `">
        <div class="channel-check">
            ...
            <label for="` + obj.user_id + `_0_chat"></label>
        </div>
        <div class="user-list-item ">
            <div class="user-avatar">
            ...
            <div class="user-info">
                ...
                <div class="chat-preview">
                    <span class="chat-is-read ` + user_status_class + `"><i class="fa fa-circle"></i> ` + user_status_class_name + `</span>
            ...
            if ('default_group' in data) {
                ...
                `<label id="" class="recent-chat " for="` + data.default_group.chat_group + `_1_chat" data-id="` + data.default_group.chat_group + `" data-name="` + data.default_group.name + `">
                    <div class="channel-check">
                        ...
                        <label for="` + data.default_group.chat_group+ `_1_chat"></label>
                    </div>
                    <div class="user-list-item ">
                        <div class="user-avatar">
                            <img class="img-profile mr-2" src="` + data.default_group.cover_url + `">
...
function forwardActionDisplay(){
    ...
    if(selected_chat_item_count > 0){
        ...
        if(selected_chat_item_count == 1){
            ...
            forward_html = forward_chat_item[0] + "和 " + (selected_chat_item_count-1) + " 个其他";
        }
...
$( document ).ready(function() {
    ...
    $(".chat-list-toggle").on('click', function(e) {
        ...
        if ($(window).width() <= 576) {
            ...
            if($('.chat-list-col').hasClass('mobile-mini-user-list')){
                ...
                $(".status-change").hide();
                $(".nav-sidebar").hide();
                $(".forward-modal .nav-sidebar").show();
    ...
    $(".message-sticker").popover({
        ...
        title : '<div class="send-sticker-txt">贴纸</div> <button type="button" id="close-popover" class="close close-popover" ><i class="fas fa-times"></i></button>',
        placement: 'top',
    });
    ...
    $(document).on('click', '.chat-link-block a', function(e) {
        ...
        var vurl = clicked_link;
        var hostname = a.hostname;
        
        if(vurl.indexOf(".m3u8") > -1 || vurl.indexOf(".mp4") > -1 || vurl.indexOf("bilibili.com") > -1 || vurl.indexOf("qq.com") > -1 || vurl.indexOf("iqiyi.com") > -1 || vurl.indexOf("youku.com") > -1 || vurl.indexOf("mgtv.com") > -1 || vurl.indexOf("sohu.com") > -1 || vurl.indexOf("cctv.com") > -1 || vurl.indexOf("pptv.com") > -1 || vurl.indexOf("le.com") > -1 ){
            e.preventDefault();
            var embedlink = "https://api.leeleo.cn/nmchat/?url=" + vurl;
            $("#video-iframe").attr('src', embedlink);
            $("#video-modal").modal();
            
        } else if (hostname == 'www.youtube.com' || hostname == 'youtube.com' || hostname == 'api.leeleo.cn') {
            var videoid = clicked_link;
            // var videoid = youtube_parser(clicked_link);
            if(videoid) {
                e.preventDefault();
                var embedlink = videoid;
    ...
    $(document).on('click', '.iframe-re', function(e) {
        var vurl = $("#video-iframe").attr('src');
        $("#video-iframe").remove();
        var html = '<iframe id="video-iframe" src="'+ vurl +'" scrolling="no" allowfullscreen="allowfullscreen" mozallowfullscreen="mozallowfullscreen" msallowfullscreen="msallowfullscreen" oallowfullscreen="oallowfullscreen" webkitallowfullscreen="webkitallowfullscreen" frameborder="0" width="100%" height="100%"></iframe>';
    ...
    $(document).on('click', '.close-dropzone', function(e) {
        ...
        $('.dropzone').toggle();
        $('.message-sticker, .message-gif').popover('hide');
        $('#file_dropzone').hide();$('#image_dropzone').hide();
        $('.chat-scroll').scrollTop($('.chat-scroll')[0].scrollHeight);
        if (is_nicescroll) {
            $(".chat-scroll").getNiceScroll().resize();
        }
    ...
    $(document).on('click', '.message-delete', function(e) {
        ...
        $(this).parent().removeClass("chat-actions-show");
        $.ajax({
            ...
            success: function(data) {
                if(data.success){
                    $('#'+message_id).find('.message-html').html(`<div class="chat-txt deleted"><i class="fa fa-ban"></i> {{_('消息已撤回')}}</div>`);
    ...
    $(document).on('click', '.message-reply', function(e) {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                var replied_html = (emojione.shortnameToImage(linkParse(htmlEncode(replied_data['current_message']))));
                var replied_preview = replied_data['current_preview'];
                ...
                if (display_name_format == 'username') {
                	...
                	var display_name = data.first_name;
                    var replied_to_short = data.first_name;
                }
                if(data.sender_id == {{ USER.id }}){
                    var replied_to = "{{_('回复你的消息')}}";
                }else{
                    var replied_to = "{{_('回复')}} "+ display_name +" {{_('的消息')}}";
                }
                    ...
                    if (data.updated_chats) {
                        $.each(data.updated_chats, function( index, obj ) {
                            ...
                            if (obj.status == 2) {
                                ...
                                $(updated_li).find('.message-html').html(`<div class="chat-txt deleted"><i class="fa fa-ban"></i> 消息已撤回</div>`);
                            }
                        });
                    }
                },
